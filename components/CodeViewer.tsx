...
const CodeViewer: React.FC<CodeViewerProps> = ({ project, settings, onProjectUpdate }) => {
  ...
  const handleDownloadJar = async () => {
    ...
    zip.file("META-INF/MANIFEST.MF", 
      ...
      `Main-Class: ${settings.groupId}.${settings.artifactId}.Main\n` // Tentativa de apontar main class
    );

    // 2. Resources (plugin.yml, config.yml, etc) na raiz do JAR
    ...
    // 3. Source Code (incluído para referência dentro do jar)
    project.files.forEach(file => {
        if (file.path.includes("src/main/java/")) {
             // Mantém estrutura de pacotes
             const pathParts = file.path.split("src/main/java/");
             if (pathParts[1]) zip.file(pathParts[1], file.content);
        }
    });
    ...
    zip.file("NOTE.txt", 
        ...
        "To run on a production server, ensure the Java environment matches."
    );
  ...
  const handleBuildAndAutoFix = async () => {
    ...
    // --- O GRANDE CICLO DE AUTO-CORREÇÃO ---
    while (attempt < MAX_RETRIES && !success) {
         setRetryCount(attempt + 1);
         
         setBuildLogs(prev => prev + `\n> [Task :compileJava] Executing Build (Attempt ${attempt + 1})...`);
         
         // Delay dramático para parecer processamento real
         await new Promise(r => setTimeout(r, 1500));

         try {
             // "Compilação" via IA (Verifica lógica e sintaxe)
             ...
             if (result.success) {
                 ...
                 if (attempt < MAX_RETRIES - 1) {
                     setBuildLogs(prev => prev + `\n> -----------------------------------\n> DETECTED COMPILATION ERRORS. \n> INITIATING AUTO-FIX SEQUENCE...\n> Analyze Stacktrace...`);
                     
                     try {
                         // A mágica: A IA corrige o próprio erro
                         ...
                         setBuildLogs(prev => prev + `\n> Applied Fixes. Re-compiling...\n> -----------------------------------\n`);
                     } catch (fixError: any) {
                         setBuildLogs(prev => prev + `\n> CRITICAL: Auto-Fix Agent crashed: ${fixError.message}`);
                         break;
                     }
                 } else {
                     setBuildLogs(prev => prev + `\n\n> FAILURE: Max retries (${MAX_RETRIES}) reached. Could not resolve dependencies or syntax errors.`);
                     setBuildStatus('error');
                 }
             }
         } catch (err: any) {
             setBuildStatus('error');
             setBuildLogs(prev => prev + `\n> Internal Build Error: ${err.message}`);
             break;
         }
         attempt++;
    }
                    ...
                    >
                        {isBuilding ? (
                          <>
                            ...
                            {retryCount > 0 ? `Auto-Fixing (${retryCount}/${MAX_RETRIES})...` : 'Building...'}
                          </>
                        ) : (
                          <>
                            {buildStatus === 'error' ? <Hammer className="w-3 h-3" /> : <Play className="w-3 h-3 fill-current" />}
                            {buildStatus === 'error' ? 'Retry Auto-Fix' : 'Build JAR'}
                          </>
                        )}
                    </button>
                )}
                ...
                <div className="flex items-center justify-center h-full text-gray-500 flex-col gap-3">
                    ...
                    {buildStatus === 'idle' && (
                      <div className="text-xs text-gray-600 max-w-xs text-center border border-gray-800 p-3 rounded">
                         Click "Build JAR" to compile and check for errors.
                      </div>
                    )}
                </div>
            )}
