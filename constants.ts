...
1. **Sistema de Build**:
   ...
   - OBRIGATÓRIO definir 'java.toolchain.languageVersion' no 'build.gradle' combinando com a versão Java solicitada para garantir compatibilidade de compilação.
...
jobs:
  build:
    ...
    # Gradle 9+ requires Java 17+ to run itself. 
    # We use Java 21 for the runner environment. 
    # The build.gradle toolchain settings (generated by AI) handle the actual plugin compilation target (e.g. Java 8, 11, 17).
